...
async def process_data(payload, path):
    ...
    for f in files_to_watch:
        try:
            wait_for_file_change(Path(path) / f, pre_hashes[f], timeout=120)
        except TimeoutError:
    ...
    if response.lower() == 'y':
        output = generate_store_payload("bug", payload.get("stack_trace", ""), diff_text, payload.get("repo", ""), curr_branch, payload.get("language", ""))
        merge_temp_branch(path, tmp_branch, curr_branch)
    else:
        rollback_to_commit(path, curr_branch, pre_commit_hash)
        output = {"message": "Rolled back to pre-fix state"}
